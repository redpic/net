<?php
	...
	use Redpic\Net\Url;
	use Redpic\Net\Cookies;
	use Redpic\Net\UserAgent;
	use Redpic\Net\ProxyServer;
	use Redpic\Net\RawHttpResponse;
	...
	class WebBrowser
	{
		...
		{
			if (!in_array($key, array('url', 'referer', 'cookies', 'proxyServer', 'networkInterface', 'userAgent', 'followLocation')))
			{
		...
		public static function MultiRequest($browsers)
		{
			...
			foreach ($browsers as $id => $browser)
			{
				...
				if (!is_null($browser->proxyServer))
				{
					...
					{
						curl_setopt($curls[$id], CURLOPT_PROXYUSERPWD, $browser->proxyServer->user . ':' . $browser->proxyServer->password);
					}
				}
